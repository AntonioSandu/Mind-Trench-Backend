...
public class GameEngineService {
    ...
    private void validateTurnRequest(
        ...
        if (trapped) {
            ...
            if (!request.getTargetNodes().isEmpty()) {
                throw new InvalidTurnException(
                        "Staying still requires no target nodes."
                );
            }
    ...
    private TurnRequest generateBossAction(Game game) {
        return BossBehaviour.generateAction(game, itemService);
    }
    ...
    private void resolveStayStill(
        ...
        logService.action(
                game,
                playerSide,
                "You focus all your energy to set yourself free from the trap, this night you're too tired to do anything else"
        );
    }
    ...
    private void resolveMove(
        ...
        for (NodeId target : action.getTargetNodes()) {
            ...
            triggeredTraps++;
            boolean shouldTrap = !stick || triggeredTraps >= 2;
            if (shouldTrap) {
                statuses.add(
                        new StatusEffect(
                                StatusEffectType.TRAPPED,
    ...
    private void removeBeartrap(
        ...
        logService.itemTriggered(
                game,
                !playerSide,
                ("A Beartrap mysteriously disappeared from node " + node)
        );
    }
    ...
    private void addSmokeWarning(
        ...
        logService.action(
                game,
                !playerSide,
                "You see smoke coming from " + state.getCurrentNode()
        );
    }
    ...
    private void giveRandomItem(Game game, boolean playerSide) {
        final int MAX_INVENTORY_NUMBER= 8;

        List<ItemType> inventory = playerSide
        ...
        Map<ItemType, Double> weights = new EnumMap<>(ItemType.class);

        for (ItemType item : items) {
            long countInInventory = inventory.stream().filter(i -> i == item).count();

            double weight = 1.0 / Math.pow(7, countInInventory);
        ...
        ItemType chosenItem = items[0];
        double cumulative = 0;

        for (Map.Entry<ItemType, Double> entry : weights.entrySet()) {
        ...
        logService.itemFound(
                game,
                playerSide,
                "You conjured a " + chosenItem.getDisplayName() + " in your sleep"
        );
    }
}
